const initializeSinglePhotoCarousels = () => {
  document.querySelectorAll("[data-single-photo-carousel]").forEach((carousel) => {
    const slides = Array.from(carousel.querySelectorAll(".single-photo-slide"));
    const previousButton = carousel.querySelector(
      ".single-photo-carousel__button--previous"
    );
    const nextButton = carousel.querySelector(
      ".single-photo-carousel__button--next"
    );
    const dots = Array.from(
      carousel.querySelectorAll("[data-single-photo-dot]")
    );
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeSinglePhotoCarousels, {
    once: true,
  });
} else {
  initializeSinglePhotoCarousels();
}
