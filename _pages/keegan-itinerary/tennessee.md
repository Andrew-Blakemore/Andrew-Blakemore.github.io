---
title: "Tennessee, United States"
permalink: /keegan-itinerary/nashville/
layout: itinerary
classes: wide custom-page
author_profile: false
header:
  image: /assets/img/photography/gallery/Driving_Through_the_615.webp
---

<div class="itinerary-story itinerary-story--tennessee">
  <header class="itinerary-story__intro">
    <p class="itinerary-story__kicker">Tennessee field notes · Designed for Living</p>
    <h2>Following prosthetics from workshop to everyday life.</h2>
    <p class="itinerary-story__dek">
      Tennessee brings clinics, fabricators, manufacturers, researchers,
      adaptive-sport programs, and prosthetic users into close proximity. This
      field file will grow as I visit those different parts of the system and
      trace how decisions made in one place are experienced in another.
    </p>

    <dl class="itinerary-story__facts">
      <div>
        <dt>Region</dt>
        <dd>Middle Tennessee</dd>
      </div>
      <div>
        <dt>First field note</dt>
        <dd>FabCo Prosthetic Designs</dd>
      </div>
      <div>
        <dt>Scope</dt>
        <dd>Fabrication, fitting &amp; daily use</dd>
      </div>
      <div>
        <dt>Project lens</dt>
        <dd>Design → device → experience</dd>
      </div>
    </dl>

    <nav class="itinerary-story__index itinerary-story__visit-index" aria-label="Tennessee organizations">
      <span aria-current="page">FabCo</span>
      <a href="#future-tennessee-visits">Amputee Blade Runners</a>
      <a href="#future-tennessee-visits">Fillauer</a>
    </nav>
    <p class="itinerary-story__visit-status">
      FabCo documented August 2026 · Additional Nashville visits planned for February
    </p>
  </header>

  <section class="itinerary-story__section itinerary-story__section--opening">
    <div class="itinerary-story__section-label">
      <span>01</span>
      <p>FabCo</p>
    </div>
    <div class="itinerary-story__opening-copy">
      <h2>A morning on the fabrication floor.</h2>
      <p class="itinerary-story__lead">
        The FabCo workshop moved between plaster and foam models, ovens, vacuum
        lines, resin, cutting tools, sanding belts, and repeated inspection.
        Each station translated information from a prosthetist into an object
        someone may depend on for most of the day.
      </p>
      <p>
        FabCo is a prosthetics-only central fabrication facility in Nashville.
        Clinics send casts, scans, models, test sockets, and instructions; the
        shop turns them into custom devices that return to the prosthetist for
        fitting. Lower-limb sockets are a large part of that work, but the floor
        also handles upper-limb devices and specialty jobs whose geometries,
        components, and performance requirements can differ substantially.
      </p>
    </div>
  </section>

  <figure class="itinerary-story__body-copy">
    <img
      src="/assets/img/travel/tennessee/fabco-finished-prosthetic-sockets-hero.webp"
      alt="Several finished prosthetic sockets standing at fabrication workstations"
      loading="lazy"
      decoding="async"
    >
    <figcaption>
      Finished sockets at FabCo show how widely materials, geometry, and final
      appearance can vary from one prescription to another.
    </figcaption>
  </figure>

  <section class="itinerary-story__section">
    <div class="itinerary-story__section-label">
      <span>02</span>
      <p>Lower-limb process</p>
    </div>
    <div>
      <h2>A general lower-limb socket fabrication pathway.</h2>
      <p class="itinerary-story__lead">
        No single sequence describes every socket. The workflow changes with
        the limb level, suspension system, material, prescription, clinic, and
        person. The three phases below show a common path through the work—not
        a claim that every FabCo job follows these exact steps.
      </p>
    </div>
  </section>

  <ol class="itinerary-story__fabrication-flow" aria-label="General lower-limb socket fabrication pathway">
    <li>
      <i class="fas fa-ruler-combined" aria-hidden="true"></i>
      <p class="itinerary-story__card-number">Phase 01 · Capture &amp; rectify</p>
      <div class="itinerary-story__process-step">
        <span>01</span>
        <div>
          <h3>Negative impression or digital scan</h3>
          <p>
            A prosthetist records the residual limb’s shape with a cast or scan,
            along with measurements and clinical observations.
          </p>
        </div>
      </div>
      <div class="itinerary-story__process-step">
        <span>02</span>
        <div>
          <h3>Positive model &amp; rectification</h3>
          <p>
            A negative cast may be filled to create a positive plaster model.
            The model—or its digital equivalent—is modified to add relief and
            support where the socket design requires it.
          </p>
        </div>
      </div>
    </li>
    <li>
      <i class="fas fa-layer-group" aria-hidden="true"></i>
      <p class="itinerary-story__card-number">Phase 02 · Verify &amp; fabricate</p>
      <div class="itinerary-story__process-step">
        <span>03</span>
        <div>
          <h3>Thermoformed diagnostic socket</h3>
          <p>
            Clear thermoplastic is vacuum formed over the model so fit,
            pressure, suspension, and alignment can be evaluated and revised
            during a supervised clinical fitting.
          </p>
        </div>
      </div>
      <div class="itinerary-story__process-step">
        <span>04</span>
        <div>
          <h3>Definitive socket &amp; wet lamination</h3>
          <p>
            Once fit is confirmed, reinforcing textiles are arranged over a
            prepared model and consolidated with resin under vacuum. Some
            designs use multiple walls or laminations; others use a definitive
            thermoplastic construction instead.
          </p>
        </div>
      </div>
    </li>
    <li>
      <i class="fas fa-tools" aria-hidden="true"></i>
      <p class="itinerary-story__card-number">Phase 03 · Finish &amp; fit</p>
      <div class="itinerary-story__process-step">
        <span>05</span>
        <div>
          <h3>Trim, finish &amp; alignment transfer</h3>
          <p>
            Trim lines are cut, edges and surfaces are refined, hardware is
            installed, and the component position established in the test setup
            can be transferred to the definitive socket.
          </p>
        </div>
      </div>
      <div class="itinerary-story__process-step">
        <span>06</span>
        <div>
          <h3>Clinical fitting &amp; dynamic alignment</h3>
          <p>
            Back at the clinic, the prosthetist evaluates comfort and movement
            with the user, adjusts alignment, and continues refining the system
            as it is used in real life.
          </p>
        </div>
      </div>
    </li>
  </ol>

  <div class="itinerary-story__feature itinerary-story__feature--uncropped">
    <figure class="itinerary-story__feature-image">
      <img
        src="/assets/img/travel/tennessee/socket-trim-line-marking.webp"
        alt="A FabCo fabricator marking trim lines on a prosthetic socket model"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Marked trim lines guide the fabricator as excess material is removed
        without losing the intended socket geometry.
      </figcaption>
    </figure>
    <div class="itinerary-story__process-note">
      <h3>The work crosses organizational boundaries.</h3>
      <p>
        The prosthetist is responsible for evaluation, socket design, fitting,
        and alignment with the user. A central fabrication shop works from that
        clinical information to make the physical device. Models, test sockets,
        measurements, photographs, and written instructions keep the design
        intact as it moves between them.
      </p>
      <p>
        That distinction became important during the visit: fabrication is not
        separate from care, but it is one specialized part of a longer process.
      </p>
    </div>
  </div>

  <div class="itinerary-story__photo-grid">
    <figure>
      <img
        src="/assets/img/travel/tennessee/thermoplastic-socket-forming.webp"
        alt="A FabCo fabricator heating and forming thermoplastic over a socket model"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Heated thermoplastic is formed around a model under vacuum to preserve
        the designed shape.
      </figcaption>
    </figure>
    <figure>
      <img
        src="/assets/img/travel/tennessee/socket-lamination-setup.webp"
        alt="A FabCo fabricator preparing a prosthetic socket for lamination"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Reinforcement and bagging are prepared before resin is drawn through
        the layup during wet lamination.
      </figcaption>
    </figure>
    <figure>
      <img
        src="/assets/img/travel/tennessee/prosthetic-socket-trimming.webp"
        alt="A FabCo fabricator trimming excess material from a prosthetic socket"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Excess material is cut away after forming, bringing the socket to its
        prescribed boundaries.
      </figcaption>
    </figure>
    <figure>
      <img
        src="/assets/img/travel/tennessee/prosthetic-socket-belt-sanding.webp"
        alt="A FabCo fabricator smoothing a prosthetic socket on a belt sander"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Sanding and edge finishing remove rough transitions before inspection
        and assembly.
      </figcaption>
    </figure>
  </div>

  <section class="itinerary-story__section">
    <div class="itinerary-story__section-label">
      <span>03</span>
      <p>Workshop knowledge</p>
    </div>
    <div>
      <h2>Much of fabrication knowledge is learned through repetition.</h2>
      <p class="itinerary-story__lead">
        Throughout the morning, fabricators brought work over for another set
        of eyes. A change in surface color, a pocket of trapped air, or an edge
        that did not match the intended line could point back to a specific
        moment in forming or lamination.
      </p>
      <p>
        The speed of those diagnoses came from accumulated examples. Written
        procedures establish the sequence, but experience helps a fabricator
        recognize what normal variation looks like, where a defect began, and
        whether it can be corrected without compromising the device. That
        knowledge also travels through phone calls, shop visits, conferences,
        and candid feedback between people working at different organizations.
      </p>
    </div>
  </section>

  <div class="itinerary-story__clinic-grid itinerary-story__clinic-grid--uncropped">
    <figure>
      <img
        src="/assets/img/travel/tennessee/prosthetic-socket-edge-finishing.webp"
        alt="A FabCo fabricator finishing the edge of an orange prosthetic socket"
        loading="lazy"
        decoding="async"
      >
      <figcaption>
        Edge finishing combines material knowledge, close visual inspection,
        and an understanding of where the socket will contact the body.
      </figcaption>
    </figure>
    <aside class="itinerary-story__field-log">
      <p class="itinerary-story__eyebrow">Observed on the floor</p>
      <ol>
        <li>
          <time>01</time>
          <div>
            <strong>Read the surface.</strong>
            <p>Color and texture can reveal trapped air, uneven heating, or another upstream process problem.</p>
          </div>
        </li>
        <li>
          <time>02</time>
          <div>
            <strong>Trace the cause.</strong>
            <p>The useful question is not only how to hide a defect, but which earlier step produced it.</p>
          </div>
        </li>
        <li>
          <time>03</time>
          <div>
            <strong>Share the correction.</strong>
            <p>Technique improves when fabricators compare results and explain what worked.</p>
          </div>
        </li>
      </ol>
    </aside>
  </div>

  <div class="itinerary-story__detail">
    <div>
      <h3>Upper-limb and specialty work follow different paths.</h3>
    </div>
    <p>
      Lower-limb sockets often move through established fabrication sequences,
      although every shape is still individual. Upper-limb and specialty
      devices can vary more widely because control method, component placement,
      suspension, range of motion, appearance, and the intended task all change
      the problem. The shop therefore relies on a flexible framework rather
      than one recipe for every device.
    </p>
  </div>

  <section class="itinerary-story__section">
    <div class="itinerary-story__section-label">
      <span>04</span>
      <p>Tools &amp; judgment</p>
    </div>
    <div>
      <h2>Digital tools stay only when their results are dependable.</h2>
      <p class="itinerary-story__lead">
        FabCo has incorporated faster digital carving and scanning that can
        preserve socket geometry as an STL file. These tools improve speed and
        documentation while leaving inspection and finishing in human hands.
      </p>
      <p>
        A 3D printer intended for test sockets offered a useful contrast. In the
        shop’s experience, inconsistent results kept it from becoming part of
        routine production. The observation was not a rejection of additive
        manufacturing; it was a reminder that a tool affecting fit must be
        repeatable on an ordinary production day, not only in an ideal trial.
      </p>
    </div>
  </section>

  <section class="itinerary-story__section">
    <div class="itinerary-story__section-label">
      <span>05</span>
      <p>Connected field</p>
    </div>
    <div>
      <h2>Fabrication is one part of a connected prosthetics system.</h2>
      <p class="itinerary-story__lead">
        FabCo’s floor felt less like an isolated vendor than one node in a
        national network. The team knew clinicians I had previously shadowed,
        contributed to adaptive-sport work, exchanged techniques with other
        specialists, and discussed projects involving companies such as
        WillowWood and Fillauer.
      </p>
      <p>
        Those relationships matter because a prosthesis crosses several kinds
        of expertise before it reaches the person who will use it: evaluation,
        design, fabrication, component engineering, fitting, and long-term
        adjustment. The person experiences the completed system, not the
        organizational boundaries behind it. Comfort problems, activity goals,
        component failures, and personal preferences become most useful when
        that information can travel back through the network.
      </p>
    </div>
  </section>

  <section class="itinerary-story__section">
    <div class="itinerary-story__section-label">
      <span>06</span>
      <p>Daily use</p>
    </div>
    <div>
      <h2>Fabrication decisions continue into the user’s day.</h2>
      <p class="itinerary-story__lead">
        A finished socket can look simple beside a microprocessor knee or
        powered hand. Yet its geometry and workmanship influence whether the
        whole system can be worn comfortably, maintained, trusted, and made
        ordinary enough to recede into the background of daily life.
      </p>
    </div>
  </section>

  <div class="itinerary-story__findings">
    <article>
      <span>01</span>
      <h3>The socket is a human interface.</h3>
      <p>
        It transfers forces while protecting living tissue, so manufacturing
        quality is inseparable from comfort, mobility, and confidence.
      </p>
    </article>
    <article>
      <span>02</span>
      <h3>Reliability shapes access.</h3>
      <p>
        A repeatable process reduces rework and delays, helping a clinic return
        a usable device to the person who needs it.
      </p>
    </article>
    <article>
      <span>03</span>
      <h3>Innovation depends on feedback.</h3>
      <p>
        New materials and tools become meaningful when users, clinicians,
        fabricators, and engineers can compare results and improve them together.
      </p>
    </article>
  </div>

  <aside class="itinerary-story__next" id="future-tennessee-visits">
    <div>
      <p class="itinerary-story__eyebrow">Questions carried forward</p>
      <h2>What I’ll look for in the next Tennessee visits.</h2>
    </div>
    <ul>
      <li>How does feedback from users move through clinicians and back to the fabrication bench?</li>
      <li>Where do newer manufacturing methods improve consistency, access, or customization?</li>
      <li>How do design priorities change across daily life, work, recreation, and adaptive sport?</li>
    </ul>
  </aside>

  <aside class="itinerary-story__sources">
    <p class="itinerary-story__eyebrow">Reporting note</p>
    <p>
      This field note combines direct observation and conversation during my
      August 3, 2026 visit with public information from
      <a href="https://www.fabcoprostheticdesigns.com/" target="_blank" rel="noopener">FabCo Prosthetic Designs</a>,
      <a href="https://www.ottobock.com/en-us/professionals/services/fabrication/tf-and-tt-test-socket-fabrication" target="_blank" rel="noopener">Ottobock test-socket fabrication</a>, and
      <a href="https://www.ottobock.com/en-us/professionals/services/fabrication/lower-limb-definitive-fabrication" target="_blank" rel="noopener">Ottobock definitive fabrication</a>.
      The process description is intentionally general because fabrication
      varies by prescription, material system, clinic, and patient.
    </p>
  </aside>

  <footer class="itinerary-story__footer">
    <div>
      <p class="itinerary-story__eyebrow">An open field file</p>
      <h2>Tennessee is the starting point, not a single visit.</h2>
      <p>
        Future entries will add clinical, manufacturing, adaptive-sport, and
        user perspectives from across the state as the project develops.
      </p>
    </div>
    <a class="itinerary-story__button" href="/keegan-traveling-fellowship/">
      Return to the journey <i class="fas fa-arrow-right" aria-hidden="true"></i>
    </a>
  </footer>
</div>
