---
title: ""
layout: single
classes: wide
---

<div class="hero-grid">

  <!-- Engineering (left-aligned default) -->
  <div class="hero-card" style="background-image: url('/assets/img/print-bed.jpg');">
    <div class="hero-overlay">
      <h2>Engineering</h2>
      <p>
        Designing, building, and troubleshooting medical devices across clinical and manufacturing environments.
      </p>
      <a href="/portfolio/" class="hero-btn">View Projects</a>
    </div>
  </div>

  <!-- Photography (right-aligned) -->
  <div class="hero-card right" style="background-image: url('/assets/img/photo.jpg');">
    <div class="hero-overlay">
      <h2>Photography</h2>
      <p>
        Documenting people, places, and process through visual storytelling.
      </p>
      <a href="/photography/" class="hero-btn">View Photography</a>
    </div>
  </div>

  <!-- Travel (left-aligned default) -->
  <div class="hero-card" style="background-image: url('/assets/img/travel.jpg');">
    <div class="hero-overlay">
      <h2>Travel</h2>
      <p>
        Exploring prosthetics and lived experiences around the world.
      </p>
      <a href="/keegan-traveling-fellowship/" class="hero-btn">View Journey</a>
    </div>
  </div>

</div>

{% include feature_row %}
